import { walletCommand } from '../command';
...
describe('postJobCommand', () => {
  ...
  it('should call run action', () => {
    walletCommand.parse(parseArgs);
    expect(mock_getAddress_action).toHaveBeenCalledTimes(1);
  });

  it('should set post job args', () => {
    walletCommand.parse(parseArgs);
    expect(walletCommand.args[0]).toBe('run command');
  });

  it('should have 2 options', () => {
    expect(walletCommand.options.length).toBe(2);
  });
  ...
  it.each([
    ...
    const option = walletCommand.options.find((i) => i.long === long);
