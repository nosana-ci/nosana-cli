...
export interface StateObserver {
    update(status: string, state: { [key: string]: string }, timestamp: number): void;
}
...
export class NodeState {
    ...
    private history: { status: string, state: { [key: string]: string; }, timestamp: number;}[] = []
    ...
    constructor( node: string ) {
        ...
        private notifyObservers(status: string, state: { [key: string]: string }, timestamp: number) {
    ...
    public process(data: LogEntry) {
        ...
        if (data.class === 'PodmanContainerOrchestration' 
            ...
            if(data.method == 'pullImage'){
                ...
                if(data.type == 'return'){
                    if(data.result.status == true){
                        this.addState('pulling-image-success', { image: data.arguments[0] })
                    } else {
                        this.addState('pulling-image-failed', { image: data.arguments[0], error: data.result.error })
                    }
                }
            }
            ...
            if(data.method == 'createNetwork'){
                ...
                if(data.type == 'return'){
                    if(data.result.status == true){
                        this.addState('creating-network-success', { network: data.arguments[0] })
                    } else {
                        this.addState('creating-network-failed', { network: data.arguments[0], error: data.result.error })
                    }
                }
            }
            ...
            if(data.method == 'deleteNetwork'){
                ...
                if(data.type == 'return'){
                    if(data.result.status == true){
                        this.addState('deleting-network-success', { network: data.arguments[0] })
                    } else {
                        this.addState('deleting-network-failed', { network: data.arguments[0], error: data.result.error })
                    }
                }
            }
            ...
            if(data.method == 'runContainer'){
                if(data.type == 'call'){
                    this.addState('running-container', { image: data.arguments[0], name: data.arguments[1].name })
                }
                ...
                if(data.type == 'return'){
                    if(data.result.status == true){
                        ...
                        this.addState('running-container-failed', { image: data.arguments[0], error: data.result.error })
                    }
                }
            }
            ...
            if(data.method == 'stopAndDeleteContainer'){
                ...
                if(data.type == 'return'){
                    if(data.result.status == true){
                        ...
                        this.addState('stopping-container-failed', { id: data.arguments[0], error: data.result.error })
                    }
                }
            }
        }
        ...
        if (data.class === 'BasicNode'){
            if(data.method == 'start'){
                ...
                if(data.type == 'error'){
                    this.addState('node-starting-failed', { node: this.shared.node, error: `${data.error}` })
                }
            ...
            if(data.method == 'stop'){
                ...
                if(data.type == 'error'){
                    this.addState('node-stopping-failed', { node: this.shared.node, error: `${data.error}` })
                }
            ...
            if(data.method == 'benchmark'){
                ...
                if(data.type == 'error'){
                    this.addState('benchmark-error', { node: this.shared.node, error: `${data.error}` })
                }
            ...
            if(data.method == 'queue'){
                ...
                if(data.type == 'call'){
                    this.addState('queueing-in-market', { node: this.shared.node, market })
                }

                if(data.type == 'error'){
                    this.addState('queueing-in-market-failed', { node: this.shared.node, market, error: `${data.error}` })
                }
    
                if(data.type == 'return'){
                    this.addState('queueing-in-market-success', { node: this.shared.node, market })
                } 
            }
        }
