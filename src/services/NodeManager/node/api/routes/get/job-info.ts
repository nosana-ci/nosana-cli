...
export function getJobInfoRoute(
  ...
  const sendJobInfoIfChanged = () => {
    try {
      ...
      const response = buildJobInfoResponse(currentFlow, task);
  ...
  const buildJobInfoResponse = (
    ...
    if (secrets && secrets[jobId]) {
      ...
      if (flowData.jobDefinition && flowData.jobDefinition.ops) {
        flowData.jobDefinition.ops.forEach((op, index) => {
      ...
      if (typeof urlsDataCandidate === 'object' && urlsDataCandidate !== null) {
        const urlsData = urlsDataCandidate as JobExposeSecrets;
        ...
        for (const [exposeId, urlData] of Object.entries(urlsData)) {
          ...
          let endpointStatus: EndpointStatus = 'OFFLINE';
