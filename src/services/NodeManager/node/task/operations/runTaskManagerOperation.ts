...
export async function runTaskManagerOperation(
    ...
    /**
     * we want to go through the env in the ops and track any env that is prefixed with "op_"
     * so for example "op_nginx" we will prefix it with the real operation podman/docker container
     * so something like "API: op_url_nginx:80" to "API: DqYLD53ER9Vjd4auZhCCiK2hspgjygUvJCH1jykyiPNf-nginx:80"
     */
    if (op.type === 'container/run') {
        const runOp = op as Operation<'container/run'>;

        if (runOp.args.env) {
            for (const key of Object.keys(runOp.args.env)) {
                const val = runOp.args.env[key];
                if (typeof val !== 'string' || !val.includes('op_url_')) continue;

                // Replace every op_url_<id>:<port> token inside the string
                const replaced = val.replace(/op_url_([A-Za-z0-9._-]+):(\d+)/g, (_m, opId, port) => {
                    const k = `${opId}:${port}`;                 // e.g. "nginx:80"
                    const mapped = this.exportMap?.get(k);       // expect something like "DqYLD...-nginx:80" or "http://DqYLD...-nginx:80"

                    // If not found, keep the original token so you can detect it later
                    if (!mapped) return _m;

                    // If your map returns only host[:port], we’re good to drop it in.
                    // If it already includes a scheme (http://), just return it as-is.
                    return mapped;
                });

                runOp.args.env[key] = replaced;
            }
        }
    }
     ...
     */
    return this.provider.runTaskManagerOperation(flow, op, abort, emitter);
}
