...
app.get('/status/:jobId', (req, res) => {
  ...
  const events = logSubscriberManager.getEvents().filter(event => event.job === jobId);
  res.write(`data: ${JSON.stringify(events)}\n\n`);

  req.on('close', () => {
    const index =  app.locals.logSatusClients.findIndex((client: StatusLogClient) => client.response === res);
    if (index !== -1) {
