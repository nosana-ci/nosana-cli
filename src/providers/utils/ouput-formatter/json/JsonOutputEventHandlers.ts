...
type EventHandler<T extends keyof OutputEventParams> = (response: JsonResponseType, param: OutputEventParams[T]) => void;
...
export const jsonOutputEventHandlers: OutputEventHandlers = {
  [OUTPUT_EVENTS.READ_KEYFILE]: (response: JsonResponseType, param: KeyfileParam) => {
    response.keypair_path = param.keyfile;
  },

  [OUTPUT_EVENTS.CREATE_KEYFILE]: (response: JsonResponseType, param: KeyfileParam) => {
    response.keypair_path = param.keyfile;
  },

  [OUTPUT_EVENTS.OUTPUT_BALANCES]: (response: JsonResponseType, param: BalanceParam) => {
    response.balances = {
      SOL: `${param.sol} SOL`,
      NOS: `${param.nos} NOS`,
    };
  ...
  [OUTPUT_EVENTS.OUTPUT_NETWORK]: (response: JsonResponseType, param: NetworkParam) => {
    response.network = param.network;
  },

  [OUTPUT_EVENTS.OUTPUT_WALLET]: (response: JsonResponseType, param: WalletParam) => {
    response.wallet = param.publicKey;
  },

  [OUTPUT_EVENTS.OUTPUT_IPFS_UPLOADED]: (response: JsonResponseType, param: IpfsParam) => {
    response.ipfs_uploaded = param.ipfsHash;
  },

  [OUTPUT_EVENTS.OUTPUT_SERVICE_URL]: (response: JsonResponseType, param: ServiceUrlParam) => {
    response.service_url = param.url;
  },

  [OUTPUT_EVENTS.OUTPUT_JOB_URL]: (response: JsonResponseType, param: JobUrlParam) => {
    response.job_url = param.job_url;
  },

  [OUTPUT_EVENTS.OUTPUT_JSON_FLOW_URL]: (response: JsonResponseType, param: JsonFlowUrlParam) => {
    response.json_flow_url = param.json_flow_url;
  },

  [OUTPUT_EVENTS.OUTPUT_MARKET_URL]: (response: JsonResponseType, param: MarketUrlParam) => {
    response.market_url = param.market_url;
  },

  [OUTPUT_EVENTS.OUTPUT_JOB_PRICE]: (response: JsonResponseType, param: JobPriceParam) => {
    response.price = `${param.price} NOS/s`;
  },

  [OUTPUT_EVENTS.OUTPUT_TOTAL_COST]: (response: JsonResponseType, param: TotalCostParam) => {
    response.total_cost = `${param.cost} NOS/s`;
  },

  [OUTPUT_EVENTS.OUTPUT_JOB_STATUS]: (response: JsonResponseType, param: JobStatusParam) => {
    response.status = param.status;
  },

  [OUTPUT_EVENTS.OUTPUT_JOB_POSTING]: (response: JsonResponseType, param: JobPostingParam) => {
  ...
  [OUTPUT_EVENTS.OUTPUT_JOB_POSTED_TX]: (response: JsonResponseType, param: TxParam) => {
    response.job_posting = {
  ...
  [OUTPUT_EVENTS.OUTPUT_JOB_VALIDATION_ERROR]: (response: JsonResponseType, param: ValidationErrorParam) => {
  ...
  [OUTPUT_EVENTS.OUTPUT_FAILED_TO_FETCH_MARKETS_ERROR]: (response: JsonResponseType, param: ErrorParam) => {
    response.isError = true;
  ...
  [OUTPUT_EVENTS.OUTPUT_JOB_POSTED_ERROR]: (response: JsonResponseType, param: JobPostedErrorParam) => {
    response.isError = true;
  ...
  [OUTPUT_EVENTS.OUTPUT_SOL_BALANCE_LOW_ERROR]: (response: JsonResponseType, param: BalanceLowParam) => {
    response.isError = true;
  ...
  [OUTPUT_EVENTS.OUTPUT_NOS_BALANCE_LOW_ERROR]: (response: JsonResponseType, param: NosBalanceLowParam) => {
    response.isError = true;
  ...
  [OUTPUT_EVENTS.OUTPUT_AIRDROP_REQUEST_FAILED_ERROR]: (response: JsonResponseType, param: ErrorParam) => {
    response.isError = true;
  ...
  [OUTPUT_EVENTS.OUTPUT_JOB_NOT_FOUND]: (response: JsonResponseType, param: JobNotFoundErrorParam) => {
    response.isError = true;
  ...
  [OUTPUT_EVENTS.OUTPUT_ARTIFACT_SUPPORT_INCOMING_ERROR]: (response: JsonResponseType, param: ErrorParam) => {
    response.isError = true;
  ...
  [OUTPUT_EVENTS.OUTPUT_JSON_FLOW_TYPE_NOT_SUPPORTED_ERROR]: (response: JsonResponseType, param: JsonFlowTypeErrorParam) => {
    response.isError = true;
  ...
  [OUTPUT_EVENTS.OUTPUT_NODE_URL]: (response: JsonResponseType, param: NodeUrlParam) => {
    response.node_url = param.url;
  },

  [OUTPUT_EVENTS.OUTPUT_DURATION]: (response: JsonResponseType, param: DurationParam) => {
    response.duration = param.duration;
  },

  [OUTPUT_EVENTS.OUTPUT_START_TIME]: (response: JsonResponseType, param: StartTimeParam) => {
    response.start_time = param.date;
  },

  [OUTPUT_EVENTS.OUTPUT_RESULT_URL]: (response: JsonResponseType, param: ResultUrlParam) => {
